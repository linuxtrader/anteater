...
# Some variables we'll use here
    our ($num_recordings, $live, $heading, $fsize, $cutlist, $plain_text, $text_format, $usage);
...
# Default status output heading
    $dheading='Recent Recording Sizes(Bytes), Commercials(mins:secs)\n';
# Default format of plain-text output
    $dtext_format='%n/%j %g:%i %A - %cc\n%T - %S\n%R\n\n';
# Default format of status output display text
    $dstatus_text_format= '<a href="#">somesize %n/%j %h:%i %A - %eh:%ei %cc - %T- %S (somecuts)<br />'.
...
# Print the output for use in the backend status page.
    sub status_print {
        my $count = shift;
        my $fsize = $rec->{'filesize'};
        1 while $fsize =~ s/(.*\d)(\d\d\d)/$1,$2/;
        my $cuts = $rec->{'cutlist'};
        $cuts =~ s/^ //;
        my @cutlist = split(/ /,$cuts);
        my $cutlsct = split(/ /,$cuts);
        my $lastm = $rec->{'lastmodified'};
        ...
        for $com (@cutlist)
        {
         ...
         my $mins=$seconds / 60;
         my $rems=$seconds % 60;
         $cut=sprintf "%d:%.2d ", $mins,$rems;
         $cutmins = "$cutmins"."$cut";
        }
        $cutmins =~ s/ $//;

        #$text =~ s/somecuts/$cutlsct/;  #Just give a count
        $text =~ s/somecuts/$cutmins/;
        $text =~ s/\(0:00 /\(/;
        $text =~ s/ 0:00\)/\)/;
        #$text =~ s/ 0:/ :/;
        #$text =~ s/\(0:/\(:/;
        #$text =~ s/\(\)//;
        $text =~ s/Untitled/movie/;
        print("$text");
